...
angular.module('message.controllers', [])
    ...
    $scope.$on('msgs.update', function (event) {
      $scope.$apply(function () {
        ...
        if($scope.userId === ''){
          ...
          // $ToastUtils.showToast("first login"+$scope.receiverssid);
  ...
  .controller('MessageGroupCtrl', function ($scope, $state, $http, $ionicScrollDelegate, $mqtt, $ionicActionSheet, $greendao, $timeout,$stateParams,$rootScope,$grouparr,$ToastUtils) {
    ...
    $scope.$on('groupMsgs.update', function (event) {
      $scope.$apply(function () {
        ...
        // $ToastUtils.showToast($scope.groupMsgs.length)
        // viewScroll.scrollBottom();
      })
  ...
  .controller('MessageCtrl', function ($scope, $http, $state, $mqtt, $chatarr, $stateParams, $rootScope, $greendao,$grouparr,$timeout,$ToastUtils,$contacts) {
    //清表数据
